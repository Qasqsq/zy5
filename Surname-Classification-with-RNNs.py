...
class SurnameVectorizer(object):
    ...
    def __init__(self, char_vocab, nationality_vocab):
        """
        参数:
            char_vocab (Vocabulary): 将字符映射为整数
    ...
    def vectorize(self, surname, vector_length=-1):
        ...
        Args:
            surname (str): 要处理的姓氏字符串
            vector_length (int): 指定输出向量的长度（-1表示自动确定）
        Returns:
            out_vector (np.ndarray): 向量化的结果
            vec_length (int): 实际有效长度
        """
...
args = Namespace(
    ...
    surname_csv="D:\RNN\surnames_with_splits.csv",
...
for surname in surnames:
    ...
    print(
        f"姓氏: {result['surname']}\n"
        f"国籍: {result['nationality']}\n"
        f"置信度: {result['probability']:.4f}\n"
    )